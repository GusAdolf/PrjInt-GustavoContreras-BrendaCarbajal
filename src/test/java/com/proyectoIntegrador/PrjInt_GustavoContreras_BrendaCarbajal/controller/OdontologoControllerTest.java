...
class OdontologoControllerTest {
    ...
    void setUp() {
        ...
        odontologo1 = new Odontologo(1L, "Juan", "Pérez");
        odontologo2 = new Odontologo(2L, "Ana", "López");
    }
    ...
    void testBuscarPorMatricula() {
        when(odontologoServicio.buscarPorMatricula(1L)).thenReturn(odontologo1);

        ResponseEntity<Odontologo> response = odontologoController.buscarPorMatricula(1L);

        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(odontologo1, response.getBody());
        verify(odontologoServicio, times(1)).buscarPorMatricula(1L);
    }
